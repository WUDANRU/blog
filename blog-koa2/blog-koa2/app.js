...
if (ENV !== 'production') {
    //开发环境/测试环境
    app.use(morgan('dev')) //开发环境日志会打印在cmd上
} else {
    //线上环境
    ...
    app.use(morgan('combined', { //线上环境日志会打印在文件上
        stream: writeStream
    }))
}
...
// session配置
app.keys = ['WJiol#23123_'] //session的密码  // 用于加密，和 express 的 secret 一样
app.use(session({ //session()执行，(引用的session,require('koa-generic-session'))
    // 配置cookie
    cookie: {
        path: '/',
        httpOnly: true,
